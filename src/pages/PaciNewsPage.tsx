...
import { createPaciNews, deletePaciNews, getPaciNews, updatePaciNews } from '../services/api';

interface PaciNewsItem {
    _id: string;
    links: {
        name: {
            en: string;
...
const PaciNewsPage = () => {
    const [news, setNews] = useState<PaciNewsItem[]>([]);
    ...
    const fetchPaciNews = async () => {
        try {
            setDataLoading(true)
            const { data } = await getPaciNews();
            setNews(data);
    ...
    const handleEdit = (item: PaciNewsItem) => {
        setFormData({
            id: item._id,
    ...
    const handleDelete = async (id: string) => {
        const isConfirmed = window.confirm('Are you sure you want to delete this news item?');
        if (isConfirmed) {
            setDataLoading(true);
            try {
                await deletePaciNews(id);
                fetchPaciNews();
            } catch (error) {
    ...
    return (
      ...
      <h1 className="p-4 text-[30px] font-bold">Paci News</h1>
